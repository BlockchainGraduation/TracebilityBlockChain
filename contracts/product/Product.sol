...
contract Product is IProduct{
    ...
    function update(string calldata id, SupplyChainLib.ProductType product_type, uint price, uint quantity, string calldata hash_info) public returns (SupplyChainLib.ProductInfo memory ){
    ...
    function burn(string calldata id, uint quantity) public returns(SupplyChainLib.ProductInfo memory){
        require(bytes(products[id].product_id).length != 0, "Product not found");
